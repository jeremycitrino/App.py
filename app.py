#!/usr/bin/env python
# app.py – Optimised REST Polling for Android APK (Termux-free)
# Uses yfinance with caching, reduced frequency.
...
# ===============================
#  FOREGROUND SERVICE (APK-compatible, no Termux)
# ===============================
def setup_persistent_notification():
    # No Termux notification in standalone APK
    logger.info("📱 Running as standalone APK – background keep-alive not guaranteed")
    logger.info("   Please keep the app open and disable battery optimization.")

def refresh_wakelock_loop():
    # No wake lock – just sleep (does nothing)
    while running:
        time.sleep(60)
...
def prevent_android_kill():
    ...
    logger.info("   1. Disable battery optimization for this app")
    logger.info("   2. Keep the app in foreground (don't swipe away)")
    logger.info("="*50)
...
def main():
    ...
    save_pid()
    prevent_sleep()
    setup_persistent_notification()   # now safe (no Termux)
